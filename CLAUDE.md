...
- Error hints: on recoverable errors (401/403 auth, keychain not found), an `errorHintItem` section appears between the header and usage rows with actionable steps (e.g. "log in at console.anthropic.com" or "start Claude Code to refresh your token"); hidden automatically on successful fetch
