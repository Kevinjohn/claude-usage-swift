...
- Progressive menu bar abbreviation: when the combined text (prefix + weekly + sonnet) exceeds ~25% of screen width, progressively shortens through 4 levels — full labels → short labels (`w:`/`s:`) → drop countdown → drop prefix. When both weekly and sonnet are displayed simultaneously, always defaults to short labels. Measured via `NSAttributedString.size().width` with 16pt button padding
